...
const app = express();
const port = 3000;
const parser = new xml2js.Parser({rootName: 'root'});
var fs = require("fs");
    ...
    async function run() {
        try {
            ...
            if (ticket === null) {
                ...
                // Convert the ticket object to XML
                const xml_str = new xml2js.Builder().buildObject(ticket);
                res.type('xml').send(xml_str); // Set response type to XML
            }
        } catch (err) {
            console.log(err);
            res.status(500).send("Error!")
